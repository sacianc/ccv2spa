...
const repeatsValues = ['Todos los dias (Lu-Vi)', 'Diario', 'Semanal', 'Mensual', 'Anual', 'Personalizar'];
...
export default class NewMessage extends React.Component<INewMessageProps, formState> {
    ...
    constructor(props: INewMessageProps) {
        ...
        this.state = {
            ...
            repeatsFor: 0,
            repeatFrequency: repeatFrequencyValues[0],
            weekSelection: "",
    ...
    public async componentDidMount() {
        ...
        repeatFrequencyValues.forEach((element) => {
            repeatFrequencydropdownValues.push({
                header: this.convertUnicode(element.valueOf()),
                team: {
                    id: this.convertUnicode(element.valueOf())
                }
            });
        });
    }
    ...
    public convertUnicode = (input: any) => {
        return input.replace(/\\u(\w\w\w\w)/g, function (a: any, b: any) {
    ...
    private getItem = async (id: number) => {
        try {
            ...
            let weekSelectionValues: boolean[] = [false, false, false, false, false, false, false];
            if (draftMessageDetail.weekSelection.indexOf('0') !== -1) {
                weekSelectionValues[0] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('1') !== -1) {
                weekSelectionValues[1] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('2') !== -1) {
                weekSelectionValues[2] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('3') !== -1) {
                weekSelectionValues[3] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('4') !== -1) {
                weekSelectionValues[4] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('5') !== -1) {
                weekSelectionValues[5] = true;
            }

            if (draftMessageDetail.weekSelection.indexOf('6') !== -1) {
                weekSelectionValues[6] = true;
            }
            ...
            this.setState({
                ...
                mondaySelection: weekSelectionValues[0],
                tuesdaySelection: weekSelectionValues[1],
                webnesdaySelection: weekSelectionValues[2],
                thursdaySelection: weekSelectionValues[3],
                fridaySelection: weekSelectionValues[4],
                saturdaySelection: weekSelectionValues[5],
                sundaySelection: weekSelectionValues[6],
    ...
    public render(): JSX.Element {
        ...
        if (this.state.IsScheduled) {
            scheduleCheckbox = <Checkbox toggle defaultChecked onClick={this.onScheduleToggle} label="SI" />;
        }
        else {
            scheduleCheckbox = <Checkbox toggle onClick={this.onScheduleToggle} label="NO" />;
        }
        if (this.state.isRecurrenceEnabled) {
            recurrenceCheckbox = <Checkbox toggle defaultChecked onClick={this.onRecurrenceToggle} label="SI" />;
        }
        else {
            recurrenceCheckbox = <Checkbox toggle={!this.state.isRecurrenceEnabled} onClick={this.onRecurrenceToggle} label="NO" />;
        }
        ...
        if (this.state.isRecurrenceEnabled) {
            ...
            if (this.state.repeats !== "Personalizar") {
                recurrenceMessage += repeats.toLowerCase();
            }
            else if (this.state.repeats === "Personalizar") {
                if (this.state.repeatFrequency === repeatFrequencyValues[0] || this.state.repeatFrequency === repeatFrequencyValues[2]) {
                    recurrenceMessage += "cada dia de la " + this.state.repeatsFor + " " + repeatFrequency.toLowerCase();
                }
                else if (this.state.repeatFrequency === repeatFrequencyValues[1]) {
                    let weeks = "";
                    ...
                    recurrenceMessage += "cada dia de la " + this.state.repeatsFor + " " + repeatFrequency.toLowerCase() + "(" + weeks.substring(0, weeks.length - 1) + ")";
                }
            }
            let repeatStartDate: string = this._onFormatDate2(this.state.repeatsStartDate);
            recurrenceMessage += " comenzando " + repeatStartDate;//.slice(repeatStartDate.length - 8)
        }
        let scheduleMessage: {} = {};
        if (this.state.IsScheduled) {
            scheduleMessage = <Text content='Seleccione fecha y hora' />
        }
        else {
            scheduleMessage = <Text content='Para agendar envio active el selector. Definir&aacute; fecha y hora' />
        }

        let recurrenceMessageControl: {} = {};
        if (this.state.isRecurrenceEnabled) {
            recurrenceMessageControl = <Text content='Elija una opci&oacute;n' />
        }
        else {
            recurrenceMessageControl = <Text content='Para activar recurrencia active el selector' />
        }
        ...
        if (this.state.weekSelection.indexOf('0') !== -1) {
            mondayCheckbox = <Checkbox label="M" defaultChecked checked onClick={(e) => this.onWeekChange(e, 0)} />
        }
        else {
            mondayCheckbox = <Checkbox label="M" onClick={(e) => this.onWeekChange(e, 0)} />
        }

        if (this.state.weekSelection.indexOf('1') !== -1) {
            tuesdayCheckbox = <Checkbox label="T" defaultChecked onClick={(e) => this.onWeekChange(e, 1)} />
        }
        else {
            tuesdayCheckbox = <Checkbox label="T" onClick={(e) => this.onWeekChange(e, 1)} />
        }

        if (this.state.weekSelection.indexOf('2') !== -1) {
            wednesdayCheckbox = <Checkbox label="W" defaultChecked onClick={(e) => this.onWeekChange(e, 2)} />
        }
        else {
            wednesdayCheckbox = <Checkbox label="W" onClick={(e) => this.onWeekChange(e, 2)} />
        }

        if (this.state.weekSelection.indexOf('3') !== -1) {
            thursdayCheckbox = <Checkbox label="T" defaultChecked checked onClick={(e) => this.onWeekChange(e, 3)} />
        }
        else {
            thursdayCheckbox = <Checkbox label="T" onClick={(e) => this.onWeekChange(e, 3)} />
        }

        if (this.state.weekSelection.indexOf('4') !== -1) {
            fridayCheckbox = <Checkbox label="F" defaultChecked onClick={(e) => this.onWeekChange(e, 4)} />
        }
        else {
            fridayCheckbox = <Checkbox label="F" onClick={(e) => this.onWeekChange(e, 4)} />
        }

        if (this.state.weekSelection.indexOf('5') !== -1) {
            saturdayCheckbox = <Checkbox label="S" defaultChecked onClick={(e) => this.onWeekChange(e, 5)} />
        }
        else {
            saturdayCheckbox = <Checkbox label="S" onClick={(e) => this.onWeekChange(e, 5)} />
        }

        if (this.state.weekSelection.indexOf('6') !== -1) {
            sundayCheckbox = <Checkbox label="S" defaultChecked onClick={(e) => this.onWeekChange(e, 6)} />
        }
        else {
            sundayCheckbox = <Checkbox label="S" onClick={(e) => this.onWeekChange(e, 6)} />
        }
                                ...
                                <Flex>
                                    <FlexItem >
                                        <Input
                                            ...
                                            label="Titulo bot&oacute;n primario"
                                            placeholder="Titulo bot&oacute;n primario"
                                    ...
                                    <FlexItem >
                                        <Input
                                            ...
                                            label="URL bot&oacute;n primario"
                                            placeholder="URL bot&oacute;n primario"
                                ...
                                <Flex>
                                    <FlexItem>
                                        <Input
                                            ...
                                            label="Titulo bot&oacute;n secundario"
                                            placeholder="Titulo bot&oacute;n secundario"
                                    ...
                                    <FlexItem>
                                        <Input
                                            ...
                                            label="ULR bot&oacute;n secundario"
                                            placeholder="ULR bot&oacute;n secundario"
                    ...
                    <div className="taskModule">
                        <div className="formContainer">
                            <div className="formContentContainer" >
                                ...
                                >
                                    <Radiobutton className="mt-2 mb-2" name="grouped" value="teams" label="Enviar al canal General de Teams espec&iacute;ficos" />
                                    <Dropdown
                                    ...
                                    />
                                    <Radiobutton className="mt-2 mb-2" name="grouped" value="rosters" label="Enviar via chat a personas espec&iacute;ficas" />
                                    <Dropdown
                                        hidden={!this.state.rostersOptionSelected}
                                        placeholder="Miembros del/los equipo(s):"
                                        search
                                        multiple
                                        items={this.getItems()}
                                        ...
                                        className="dropDownmrg"
                                    />
                                    <Radiobutton className="mt-2 mb-2" name="grouped" value="allUsers" label="Enviar via chat a todos" />
                                    ...
                                    <Radiobutton className="mt-2 mb-2" name="grouped" value="adgroups" label="Enviar a un grupo de distribuci&oacute;n" />
                                    <Dropdown
                                        hidden={!this.state.adGroupOptionSelected}
                                        placeholder="Ingrese el alias del grupo de distribuci&oacute;n"
                                        search
                                        multiple
                                ...
                                <div className="contentwrapper">
                                    ...
                                    <Flex gap="gap.large" className="mb-2">
                                        <Text content="Agendar env&iacute;o" weight="bold" />
                                    ...
                                    <Divider className="pt-2 pb-2" />
                                    <Flex gap="gap.large" className="mb-2">
                                        <Text content="Mensaje recurrente" weight="bold" />
                                        <div className="ml-auto">{recurrenceCheckbox}</div>
                                    </Flex>
                                    {recurrenceMessageControl}
                                    <Dropdown
                                        className="mb-3"
...
"
                                    ...
                                    <Flex className="weekdaywrapper">
                                        <span hidden={!this.state.isRecurrenceEnabled || this.state.repeats.toString() !== repeatsValues[5] || this.state.repeatFrequency !== repeatFrequencyValues[1]}>
                                            {mondayCheckbox}
                                            {tuesdayCheckbox}
                                            {wednesdayCheckbox}
                                            {thursdayCheckbox}
                        ...
                        <div className="footerContainer">
                            <div className="buttonwrap">
                                <Flex gap="gap.small">
                                    <Button className="plainbtn" text content="< Volver" onClick={this.onBack} />
                                    <FlexItem push>
                                        <Button content="Guardar como borrador" disabled={this.isSaveBtnDisabled()} id="saveBtn" onClick={this.onSave} />
                                    </FlexItem>
                                    <Loader id="sendingLoader" className="hiddenLoader sendingLoader" size="smallest" label="Preparando mensaje" labelPosition="end" />
                                    <Button content="Enviar" disabled={this.isSaveBtnDisabled()} id="saveBtn" onClick={this.onSend} primary />
                                </Flex>
                            </div>
                        </div>
                    </div>
    ...
    private onWeekChange = (e: React.SyntheticEvent<HTMLElement, Event>, weekday: number) => {
        ...
        this.setState({
            ...
            () => {
                if (this.state.repeats === repeatsValues[5] && this.state.repeatFrequency === repeatFrequencyValues[1] && !this.state.mondaySelection && !this.state.tuesdaySelection && !this.state.tuesdaySelection && !this.state.webnesdaySelection && !this.state.thursdaySelection && !this.state.fridaySelection && !this.state.saturdaySelection && !this.state.sundaySelection) {
                    this.setState({
    ...
    private onRepeatFrequencyChange = (event: any, itemsData: any) => {
        this.setState({
            ...
            () => {
                if (this.state.repeats === repeatsValues[5] && this.state.repeatFrequency === repeatFrequencyValues[1] && !this.state.mondaySelection && !this.state.tuesdaySelection && !this.state.webnesdaySelection && !this.state.thursdaySelection && !this.state.fridaySelection && !this.state.saturdaySelection && !this.state.sundaySelection) {
                    this.setState({
                        isWeeksSelectionValid: false,
                    })
                }
    ...
    private onImageLinkChanged = (event: any) => {
        ...
        if (!((url === "") || (url.startsWith("https://") || (url.startsWith("data:image/png;base64,")) || (url.startsWith("data:image/jpeg;base64,")) || (url.startsWith("data:image/gif;base64,"))))) {
            this.setState({
                errorImageUrlMessage: "La URL debe comenz&aacute;r con https://"
            });
        } else {
            this.setState({
                errorImageUrlMessage: ""
