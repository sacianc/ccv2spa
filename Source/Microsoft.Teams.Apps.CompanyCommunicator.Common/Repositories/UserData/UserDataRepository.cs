        ...
        public async Task UpdateDepartmentAsync(string partitionKey)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(partitionKey))
                {
                    ...
                    IEnumerable<UserDataEntity> userDataEntities2 = await this.GetAllAsync(partitionKey);
                    List<UserDataEntity> userDataEntitiesList2 = userDataEntities2.ToList();

                    foreach (var user in usersDataBatchResults)
                    {
                        string dept = userDataEntitiesList2.FirstOrDefault(x => x.AadId == user.AadId).Department;
                        string dept2 = user.Department;
                        if (userDataEntitiesList2.FirstOrDefault(x => x.AadId == user.AadId).Department != user.Department)
                        {
